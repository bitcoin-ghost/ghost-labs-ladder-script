...
{
    std::vector<uint256> leaves;
    leaves.reserve(proof.rungs.size());
                               ...
                               std::string& error)
{
    ...
    // n_rungs (varint — read as CompactSize)
    if (pos >= data.size()) { error = "truncated creation proof: n_rungs"; return false; }
    uint64_t n_rungs = data[pos++];
    if (n_rungs >= 253) {
        // Extended CompactSize not expected for reasonable rung counts
        error = "creation proof: too many rungs (CompactSize encoding)";
        return false;
    }

    proof.rungs.resize(n_rungs);

    for (uint64_t r = 0; r < n_rungs; ++r) {
        auto& rung = proof.rungs[r];

        // n_blocks
        if (pos >= data.size()) { error = "truncated creation proof: n_blocks at rung " + std::to_string(r); return false; }
        uint8_t n_blocks = data[pos++];

        rung.blocks.resize(n_blocks);
        for (uint8_t b = 0; b < n_blocks; ++b) {
            if (pos + 3 > data.size()) { error = "truncated creation proof: block data"; return false; }
            uint16_t block_type = data[pos] | (static_cast<uint16_t>(data[pos + 1]) << 8);
            uint8_t inverted = data[pos + 2];
            rung.blocks[b] = {block_type, inverted};
            pos += 3;
        }

        // Coil: type(1) + attestation(1) + scheme(1) + output_index(1) + has_address(1)
        if (pos + 5 > data.size()) { error = "truncated creation proof: coil data"; return false; }
        rung.coil.coil_type = static_cast<RungCoilType>(data[pos]);
        rung.coil.attestation = static_cast<RungAttestationMode>(data[pos + 1]);
        rung.coil.scheme = static_cast<RungScheme>(data[pos + 2]);
        rung.coil.output_index = data[pos + 3];
        // has_address byte at pos+4 (we read but don't store address in creation proof)
        pos += 5;

        // value_commitment (32 bytes)
        if (pos + 32 > data.size()) { error = "truncated creation proof: value_commitment"; return false; }
        memcpy(rung.value_commitment.data(), &data[pos], 32);
        pos += 32;
    }

    if (pos != data.size()) {
        error = "creation proof: trailing bytes (" + std::to_string(data.size() - pos) + ")";
        return false;
    }
...
std::vector<uint8_t> SerializeCreationProof(const CreationProof& proof)
{
    ...
    // n_rungs (CompactSize — single byte for < 253)
    out.push_back(static_cast<uint8_t>(proof.rungs.size()));

    for (const auto& rung : proof.rungs) {
        // n_blocks
        out.push_back(static_cast<uint8_t>(rung.blocks.size()));
        ...
        // value_commitment
        out.insert(out.end(), rung.value_commitment.begin(), rung.value_commitment.end());
    }
                            ...
                            std::string& error)
{
    if (proof.rungs.empty()) {
        error = "creation proof: no rungs";
        return false;
    }

    // Track which outputs have at least one rung assigned
    std::vector<bool> output_covered(n_outputs, false);

    for (size_t r = 0; r < proof.rungs.size(); ++r) {
        const auto& rung = proof.rungs[r];

        // Validate each block's type and inversion flag
        for (size_t b = 0; b < rung.blocks.size(); ++b) {
            ...
            if (!IsKnownBlockType(block_type)) {
                error = "creation proof rung " + std::to_string(r) +
                        ...
                        HexStr(std::span<const uint8_t>{reinterpret_cast<const uint8_t*>(&block_type), 2});
                return false;
            }

            if (inverted && !IsInvertibleBlockType(bt)) {
                error = "creation proof rung " + std::to_string(r) +
                        " block " + std::to_string(b) +
                        ": non-invertible block type with inverted flag";
                return false;
            }
            ...
            if (inverted > 1) {
                error = "creation proof rung " + std::to_string(r) +
                        " block " + std::to_string(b) +
                        ": invalid inverted flag value " + std::to_string(inverted);
                return false;
            }
        }

        // Validate coil
        if (!IsKnownCoilType(static_cast<uint8_t>(rung.coil.coil_type))) {
            error = "creation proof rung " + std::to_string(r) + ": unknown coil type";
            return false;
        }
        if (!IsKnownAttestationMode(static_cast<uint8_t>(rung.coil.attestation))) {
            error = "creation proof rung " + std::to_string(r) + ": unknown attestation mode";
            return false;
        }

        // Validate output_index
        if (rung.coil.output_index >= n_outputs) {
            error = "creation proof rung " + std::to_string(r) +
                    ": output_index " + std::to_string(rung.coil.output_index) +
                    " >= n_outputs " + std::to_string(n_outputs);
            return false;
        }
        output_covered[rung.coil.output_index] = true;
    }

    // Every non-DATA_RETURN output must have at least one rung
    // (DATA_RETURN outputs have value 0 — caller must exclude them from n_outputs
    //  or mark them separately. For now, require all outputs covered.)
    for (size_t i = 0; i < n_outputs; ++i) {
        if (!output_covered[i]) {
            error = "creation proof: output " + std::to_string(i) + " has no rung assigned";
            return false;
        }
    }

    // Compute root from creation proof and verify it matches
    uint256 computed_root = ComputeTxMLSCRoot(proof);
    if (computed_root != expected_root) {
        error = "creation proof: root mismatch — computed " +
                computed_root.GetHex() + " != expected " + expected_root.GetHex();
        return false;
    }
